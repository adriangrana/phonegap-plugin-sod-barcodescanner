...
//------------------------------------------------------------------------------
@interface CDVbcsViewController : UIViewController <CDVBarcodeScannerOrientationDelegate,UITextFieldDelegate> {}
...
- (UIView*)buildOverlayView {
    ...
    CGRect bounds1 = self.view.bounds;
    bounds1 = CGRectMake(0, 95, bounds1.size.width, bounds1.size.height-95);
    ...
    UIToolbar* statusbar = [[UIToolbar alloc] init];
    UIToolbar* lightbar = [[UIToolbar alloc] init];
    UIToolbar* titlebar = [[UIToolbar alloc] init];
    UIToolbar* skubar = [[UIToolbar alloc] init];
    UIToolbar* descriptionBar = [[UIToolbar alloc] init];
    statusbar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    lightbar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    titlebar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    skubar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    descriptionBar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    ...
    id TitleButton = [[[UIBarButtonItem alloc]
                        initWithTitle:@"Lector" style:UIBarButtonItemStylePlain
                        target:self
                        action:nil
                        ] autorelease];
    ...
    //FINDER VEW
    CGRect someRect = CGRectMake(0.0, 0.0,  CGRectGetWidth(bounds)-30, 27.0f);
    UIView* finderView =  [[UIView alloc] initWithFrame:someRect];
    finderView.layer.cornerRadius = 5.0;
    finderView.backgroundColor = [UIColor colorWithDisplayP3Red:235.0f/255 green:235.0f/255  blue:235.0f/255  alpha:1];
    
    //FINDER CONTENT TEXTFIELD
    CGRect skuRect = CGRectMake(30.0, 0.0,  CGRectGetWidth(bounds)-60, 27.0f);
    UITextField* skuButton = [[[UITextField alloc] initWithFrame:skuRect] autorelease];
    
    skuButton.delegate = self;
    [skuButton setReturnKeyType:UIReturnKeyDone];
   ...
   // searchView.image = search1;
    ...
    // DESCRIPTION LABEL
    CGRect labelect = CGRectMake(0.0, 0.0,  CGRectGetWidth(bounds)-30, 40.0f);
    UILabel* labelInfo = [[[UILabel alloc] initWithFrame:labelect] autorelease];
    [labelInfo setText:@"Centra el codigo de barras para agregar productos o leer codigos QR"];
   
    labelInfo.textColor = [UIColor whiteColor];
    labelInfo.lineBreakMode = NSLineBreakByWordWrapping ;
    labelInfo.numberOfLines = 2;
    labelInfo.font = [UIFont systemFontOfSize:16.0];
    ...
    //ITEMS
    statusbar.items =[@[flexSpace] mutableCopy];
    lightbar.items = [@[flexSpace, TorchItem, flexSpace] mutableCopy];
    titlebar.items = [@[backButton,flexSpace, TitleButton, flexSpace] mutableCopy];
    skubar.items = [@[flexSpace,SearchItem,flexSpace] mutableCopy];
    descriptionBar.items = [@[labelItem] mutableCopy];
    bounds = overlayView.bounds;

   
    //CONFIG
    [skubar setBackgroundColor:[UIColor whiteColor]];
    [titlebar setBackgroundColor:[UIColor whiteColor]];
    [statusbar setBackgroundColor:[UIColor whiteColor]];
    titlebar.alpha = 1;
    skubar.alpha = 1;
    statusbar.alpha = 1;
    [lightbar setTintColor:[UIColor whiteColor]];
    [titlebar setTintColor:[UIColor darkGrayColor]];
    
    [lightbar setBackgroundImage:[UIImage new] forToolbarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
    [lightbar setShadowImage:[UIImage new] forToolbarPosition:UIBarPositionAny];
    [descriptionBar setBackgroundImage:[UIImage new] forToolbarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
    [descriptionBar setShadowImage:[UIImage new]
    ...
    [titlebar sizeToFit];
    [skubar sizeToFit];
    [descriptionBar sizeToFit];
    [statusbar sizeToFit];
    
    CGFloat toolbarHeight  = [lightbar frame].size.height;
    CGFloat rootViewHeight = CGRectGetHeight(bounds);
    CGFloat rootViewWidth  = CGRectGetWidth(bounds);
    
    CGRect  rectArea       = CGRectMake(0, rootViewHeight - toolbarHeight*2+15, rootViewWidth, toolbarHeight);
    CGRect  titlerectArea   = CGRectMake(0, 10, rootViewWidth, toolbarHeight);
    CGRect  skurectArea      = CGRectMake(0, toolbarHeight+8, rootViewWidth, toolbarHeight);
    CGRect  statusrectArea      = CGRectMake(0,0, rootViewWidth, 10);
    CGRect  descriptionrectArea = CGRectMake(0, rootViewHeight - toolbarHeight*3, rootViewWidth, toolbarHeight);
    
    [lightbar setFrame:rectArea];
    [titlebar setFrame:titlerectArea];
    [skubar setFrame:skurectArea];
    [statusbar setFrame:statusrectArea];
  
    [descriptionBar   setFrame:descriptionrectArea];
    

    [overlayView addSubview: titlebar];
    [overlayView addSubview: skubar];
   ...
   CGRect layerArea = CGRectMake(
                          (CGFloat) (0.5 * (rootViewWidth  - minAxis))+((rootViewWidth-250)/2),
                          (CGFloat) (0.5 * (rootViewHeight - minAxis))+((rootViewWidth-250)/2),
                                 250,
                                 250
                       
                          );
        UIBezierPath *Big =
    [UIBezierPath bezierPathWithRect:bounds1];
    
    UIBezierPath *Small =
    [UIBezierPath bezierPathWithRect:layerArea];
    
    [Big appendPath:Small];
    Big.usesEvenOddFillRule = true;
    
    
    CAShapeLayer *fillLayer = [CAShapeLayer layer];
    fillLayer.path = Big.CGPath;
    ...
    [overlayView addSubview: descriptionBar];
    [overlayView addSubview: lightbar];
    

 //   [overlayView addSubview: opaqueView];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    [_processor barcodeScanDone:^{
        [_processor.plugin returnSuccess:textField.text format:@"CODE_128" cancelled:FALSE flipped:FALSE callback:_processor.callback];
    }];
    NSMutableDictionary* resultDict = [[NSMutableDictionary new] autorelease];
    resultDict[@"text"] = textField.text;
    resultDict[@"format"] = @"CODE_128";
    resultDict[@"cancelled"] = 0;
    CDVPluginResult* result = [CDVPluginResult
                               resultWithStatus: CDVCommandStatus_OK
                               messageAsDictionary: resultDict
                               ];
    [_processor.plugin.commandDelegate sendPluginResult:result callbackId:_processor.callback];
    return NO;
}
